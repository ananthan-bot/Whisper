...
export default function Navbar() {
  const { viewMode, setViewMode, tasks, user, logout } = useStore();
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-full">
            {['requester', 'helper'].map((mode) => (
              <button
                ...
                className={cn(
                  'px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-300 capitalize cursor-pointer',
                  viewMode === mode
          ...
          {/* User Auth Buttons */}
          {user ? (
            <div className="flex items-center gap-2 border border-slate-200 rounded-full px-3 py-1 bg-slate-50">
              <User className="w-3.5 h-3.5 text-primary-600" />
              <span className="text-xs font-semibold text-slate-700">{user.alias}</span>
              <button
                onClick={logout}
                title="Log out"
                className="text-slate-400 hover:text-rose-500 ml-1 p-0.5 transition-colors cursor-pointer"
              >
            ...
            <button
              ...
              className="text-xs font-semibold text-slate-600 hover:text-primary-600 px-3 py-1.5 rounded-full border border-slate-200 bg-white hover:bg-slate-50 transition-colors cursor-pointer"
            >
              Sign In
            </button>
          )}
